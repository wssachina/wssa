...
class Registration extends ActiveRecord {
	...
	public function getTShirtSizeText() {
		...
		return isset($sizes[$this->t_shirt_size]) ? $sizes[$this->t_shirt_size] : '';
	}

	public function getStaffTypeText() {
		$types = self::getStaffTypes();
		return isset($types[$this->staff_type]) ? $types[$this->staff_type] : '';
	}
	...
	public function getOperationButton() {
		...
		$canApprove = Yii::app()->user->checkRole(User::ROLE_ADMINISTRATOR);
		if ($canApprove) {
			switch ($this->status) {
	...
	public function createPay() {
		...
		$pay->order_name = $this->competition->getFullName() . '-' . $this->user->name_zh;
		$r = $pay->save();
		return $pay;
	}
